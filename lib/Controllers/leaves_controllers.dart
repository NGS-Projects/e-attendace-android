...
class LeavesController extends GetxController {
  ...
  TextEditingController EAleaveReasonController = TextEditingController();
  // EAttendance
  var applyLeaveLoading = false.obs;

  Future<void> applyLeave(Map payload, File file) async {
    //vvip
    applyLeaveLoading(true);

    try {
      //
      var response = await apiService.postRequestwithImageandToken(
          endpoint: "leaves/apply", payload: payload, profilePic: file);
      //auth/register
      ///////////////////////////////////NA
      ...
      print(response);
      if (data["message"] == "Leave application submitted successfully") {
        // Get.toNamed(KSignUp);
  ...
  //Leaves History
  var leavesList = [].obs;
  var selectleaves = {}.obs;
  var leavesListLoading = false.obs;

  Future<void> empLeavesList() async {
    ...
    try {
      ...
      if (data == null) {
        ...
        //   msg: "Successful",
        // );
        leavesList.value = data;
        print(leavesList.value);
...
// update Leave status
  var approveLeaveLoading = false.obs;
  ...
  Future<void> approveEmployees(Map payload, String id) async {
    ...
    try {
      var response = await apiService.patchRequest(
          ...
          endpoint: "hr/leaves-status-change/${id}",
          // /:leaveId
  ...
  var selectedapprovalleavesList = {}.obs;
  var approvalleavesListLoading = false.obs;

  Future<void> hrapprovalleavesList() async {
    approvalleavesListLoading(true);
    try {
      var response = await apiService.getter(
          //https://new-ram-blood-server.onrender.com
          endpoint:
      ...
      if (data == null) {
        ...
        //   msg: "Successful",
        // );
        approvalleavesList.value = data;
        print(approvalleavesList.value);
        //assetsData.value = data;
        //   assetsDatafilter.value = data;
        //   OriginalassetsData.value = data;
        //donorsData.value = data;
      ...
      approvalleavesListLoading(false);
    }
  }
